...
describe("publicCacheKey", () => {
  it("builds normalized https url from host and path", () => {
    expect(publicCacheKey("Warung.Tokoweb.ID", "/menu")).toBe("https://warung.tokoweb.id/menu");
  });

  it("defaults empty path to root", () => {
    expect(publicCacheKey("warung.tokoweb.id", "")).toBe("https://warung.tokoweb.id/");
  });
});
