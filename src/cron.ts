...
import { cancelStaleNewOrders, cancelStaleUnpaidOrders } from "@/db/orders";
...
import { STALE_NEW_HOURS, staleUnpaidCutoffIso } from "@/domain/order";
...
export async function runNightlyMaintenance(env: Bindings, nowMs: number): Promise<void> {
  ...
  await cancelStaleNewOrders(env.DB, staleUnpaidCutoffIso(nowMs, STALE_NEW_HOURS));
}
